...
const SCHEMA = `
  ...
  -- equivalent knobs barktown-utils' reanalyze_clip.py takes as CLI flags).
  -- param_id is the same snake_case name used in barktown-goblin's cfg dict
  -- and reanalyze_clip.py's --flag names, so no translation table is needed.
...
/** Map of paramId -> currentValue, the shape barktown-goblin and reanalyze_clip.py consume. */
...
export function getHitMetadata(db, clipId) {
  const row = db.prepare(
    `SELECT h.clip_id AS clipId, d.date, h.timestamps, h.confidences, h.loudnesses,
            h.padding_s AS paddingS, h.window_s AS windowS, h.created_at AS createdAt
     FROM hit_metadata h
