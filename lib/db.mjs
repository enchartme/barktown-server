...
/**
 * Repair review fragments created while diary moves incorrectly used the
 * recording padding as their length. Offline re-analysis records zero padding,
 * so those rows were persisted as points even though they represent the
 * default 1.5-second inference window. The hit end-anchor is still intact.
 */
function repairZeroDurationReviewFragments(db) {
  db.prepare(`
    UPDATE annotations
    SET start_sec = MAX(0, end_sec - 1.5)
    WHERE source = 'model'
      AND label = 'review'
...
export function openDb(dbPath) {
  ...
  repairZeroDurationReviewFragments(db);
...
/**
 * Replace classifier-owned training targets atomically. Human/model bark,
 * review, and yap fragments are invalidated by the new inference result;
 * notes and all other fragment labels are preserved.
 */
export function replaceSampleAnalysisFragments(db, sampleId, barkFragments) {
  if (!Array.isArray(barkFragments)) throw new TypeError("barkFragments must be an array");
  return db.transaction(() => {
    if (!getSample(db, sampleId)) throw new Error(`sample not found: ${sampleId}`);
    db.prepare(`
      DELETE FROM annotations
      WHERE sample_id = ?
        AND source <> 'note'
        AND label IN ('bark', 'review', 'yap')
    `).run(sampleId);
    for (const fragment of barkFragments) insertAnnotation(db, sampleId, fragment);
    return listAnnotations(db, sampleId);
  })();
}
...
/**
 * Commit the database effects of one successful manual re-analysis together.
 * New classifier output invalidates the operator's review of the previous
 * result, so approval is cleared in the same transaction as metadata/trim.
 */
export function saveReanalysisResult(db, clipId, hitMetadata, {
  trimStartMs = null,
  trimStopMs = null,
} = {}) {
  return db.transaction(() => {
    upsertHitMetadata(db, clipId, hitMetadata);
    const trimmedEntry = setDiaryTrim(db, clipId, { trimStartMs, trimStopMs });
    if (!trimmedEntry) throw new Error(`diary entry not found: ${clipId}`);
    const unapprovedEntry = setDiaryApproved(db, clipId, false);
    if (!unapprovedEntry) throw new Error(`diary entry not found: ${clipId}`);

    return {
      ...getHitMetadata(db, clipId),
      trimStartMs: unapprovedEntry.trimStartMs,
      trimStopMs: unapprovedEntry.trimStopMs,
      approved: unapprovedEntry.approved,
    };
  })();
}
