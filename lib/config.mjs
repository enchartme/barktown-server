...
export function buildConfig() {
  return {
    ...
    // "Re-analyze" feature: spawns barktown-goblin's tools/analyze_wav.py
    ...
    // hit_metadata. Assumes barktown-goblin is checked out next to this repo
    ...
    reanalyze: {
      ...
      scriptPath: process.env.REANALYZE_SCRIPT_PATH
        ?? new URL("../../barktown-goblin/tools/analyze_wav.py", import.meta.url).pathname,
      modelDir: process.env.REANALYZE_MODEL_DIR
        ?? new URL("../../barktown-goblin/models", import.meta.url).pathname,
      timeoutMs: parseInt(process.env.REANALYZE_TIMEOUT_MS ?? "300000", 10),
      maxStdoutBytes: parseInt(process.env.REANALYZE_MAX_STDOUT_BYTES ?? "2097152", 10),
      maxStderrBytes: parseInt(process.env.REANALYZE_MAX_STDERR_BYTES ?? "131072", 10),
      concurrency: parseInt(process.env.REANALYZE_CONCURRENCY ?? "4", 10),
    },
  };
}
