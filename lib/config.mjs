...
export function buildConfig() {
  return {
    ...
    reanalyze: {
      ...
      // Detection tuning, forwarded to reanalyze_clip.py's matching CLI
      // flags. These are just the defaults — POST /api/diary/:id/reanalyze
      // accepts a body with the same fields (camelCase) to override them
      // per-request (e.g. a UI slider), see server.mjs.
      candidateThreshold: parseFloat(process.env.REANALYZE_CANDIDATE_THRESHOLD ?? "0.92"),
      hitRefractoryS:     parseFloat(process.env.REANALYZE_HIT_REFRACTORY_S ?? "1.5"),
      inferenceWindowS:   parseFloat(process.env.REANALYZE_INFERENCE_WINDOW_S ?? "1.5"),
      scoreIntervalS:     parseFloat(process.env.REANALYZE_SCORE_INTERVAL_S ?? "0.25"),
    },
  };
}
