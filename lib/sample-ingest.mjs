...
/** Existing durable sample objects must not be regenerated on every poll. */
export function needsLegacySampleIngest(objectKey, plan, existing) {
  if (!plan || plan.error) return true;
  return !existing || existing.audioPath !== objectKey;
}
